...
- **macOS** (`plan §21`): `MenuBarExtra(.window)` as the primary and, in MVP 1, only surface. `LSUIElement` accessory activation policy so no Dock icon appears.
- **iOS** (`plan §22`): `TabView` with `.tabViewStyle(.sidebarAdaptable)` (iOS 18 — exactly the floor). Tabs: Today · Pool · Timer · Settings.
