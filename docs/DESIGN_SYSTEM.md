...
- Task editing as a sheet (title, notes, scheduled day, priority, start-timer-for-task, **Time Spent**
  ...
  Pool uses Reminders-style category cards for Today, All Tasks, Archived, and Completed, plus sort
  (newest, oldest, title, priority). Archived is currently a UI category over completed tasks because
  the task model has no separate archived field.
