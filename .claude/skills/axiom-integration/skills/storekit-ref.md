...
Server-signed payloads can additionally carry `offerType: 5` (retention offer, WWDC 2026) — no client `OfferType` case exists for it as of the 27 beta 5 SDK; see Retention Messaging API below.
...
`RedeemOption` is `Equatable` / `Hashable` / `Sendable`, but **no public option values ship as of the Xcode 27 beta 5 SDK** — pass `[]` (the UIKit/AppKit variants default it) and re-check later betas for concrete options. Not available on tvOS or watchOS.
