...
- **CarPlay Simulator** (standalone Mac app) — closest match to real-vehicle behavior. Install via Xcode → Open Developer Tool → More Developer Tools… (downloads the *Additional Tools for Xcode* package; CarPlay Simulator is in the Hardware folder). Supports screen-size configuration, light/dark content style, and instrument cluster (navigation apps).
  ...
  **It requires a physical iPhone over USB and cannot drive a booted simulator.** Verified structurally: the binary links `MobileDevice.framework` and not CoreSimulator, and it tells you so — *"To edit the CarPlay configuration, first connect iPhone to the Mac … with a USB cable"*. This is a scheduling constraint, not a detail: on Xcode 27 it is the **only** CarPlay path, so a device-free testing session cannot happen. Confirm hardware is available before planning one.
- **Xcode Simulator** — quick iteration via Simulator menu → I/O → External Displays → CarPlay. Enable extra options: `defaults write com.apple.iphonesimulator CarPlayExtraOptions -bool YES`. Limitations: no instrument cluster, no locked-iPhone testing, no FM-radio interruption, no Siri features. **Both the menu path and the `CarPlayExtraOptions` default are Xcode 26 and earlier** — Xcode 27 ships no `Simulator.app`, and the key appears nowhere in that toolchain (verified). On 27, CarPlay simulation moved into Device Hub as a DeviceKit plugin (`CarPlaySimulator.devicekitplugin`).
